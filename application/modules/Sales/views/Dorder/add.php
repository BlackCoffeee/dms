            ...
            <div class="card-body">
                <form action="<?= base_url($linkin . '/addAction/' . $this->uri->segment(4)) ?>" method="post" class="needs-validation" enctype="multipart/form-data" novalidate>
                    <div class="form-group mb-3">
                        ...
                        <select name="idBarang" required class="form-control">
                            <?php foreach ($stok as $stok) : ?>
                                <option <?= $stok->stok == '0' ? 'disabled' : '' ?> value="<?= $stok->idBarang ?>"><?= $stok->merk . ' ' . $stok->namaBarang . ' | Stok =' . $stok->stok . ' ' . $stok->satuan . ' | Harga = ' . rupiah($stok->hargaSatuan) ?></option>
                            <?php endforeach ?>
                        </select>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="number" name="jumlahOrder" class="form-control" required>
                        <div class="invalid-feedback">
                            Harus diisi!
                        </div>
                    </div>
